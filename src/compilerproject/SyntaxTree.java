...
public class SyntaxTree {
 ...
 private enum NodeType {EXP, STATEMENT};
	...
	public void addChild(int parentNode, int childNode){
        ...
        public void addChild(int parentNode, int childNode, String edgeColor) {
    String edge = "n" + parentNode + " -> " + "n" + childNode + " [color=\"" + edgeColor + "\"];";
    graph.addln(edge);
}
	...
	private int makeNode(String label, NodeType type){
		String shape = (type == NodeType.EXP)? "circle" : "box";
                if(NodeType.EXP == type){
                String node = "n" + nodeCounter + " [label=\"" + label + "\", " + "shape="  + shape + "   ordering=\"out\"]"+"];";
                }
